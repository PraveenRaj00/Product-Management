pipeline {
    ...
    stages {
        ...
        stage('Test') {
            steps {
                script {
                    sh 'nohup mvn test -f Product-microservice/pom.xml > Product-microservice-test.log 2>&1 &'
                    sh 'nohup mvn test -f Order-microservice/pom.xml > Order-microservice-test.log 2>&1 &'
